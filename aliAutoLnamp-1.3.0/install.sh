...
if echo $web |grep "nginx" > /dev/null;then
  read -p "Please select the nginx version of 1.0.15/1.2.5/1.4.4, input 1 or 2 or 3 : " tmp
  if [ "$tmp" == "1" ];then
  ...
  read -p "Please select the apache version of 2.2.22/2.4.2, input 1 or 2 : " tmp
  if [ "$tmp" == "1" ];then
...
tmp=1
read -p "Please select the php version of 5.3.18/5.4.23/5.5.7, input 1 or 2 or 3 : " tmp
if [ "$tmp" == "1" ];then
  php_version=5.3.18
elif [ "$tmp" == "2" ];then
  php_version=5.4.23
...
tmp=1
read -p "Please select the mysql version of 5.1.73/5.5.35/5.6.15, input 1 or 2 or 3 : " tmp
if [ "$tmp" == "1" ];then
